...
function initMap() {
  ...
  function getWeatherInfo(latitude, longitude){
    var data_file = "http://api.openweathermap.org/data/2.5/weather?lat=" + latitude + "&lon=" + longitude + "&units=metric&APPID=bcc4494fab8d013bf08572bf17c0b460";
 ...
 function createCard(lati, long, json){ 
    ...
    if(json.weather[0].description == "clear sky" && json.main.temp > 20){
        infoWindow.setContent(
            ...
            '<h3>' + "YES, It's sunny here and " + json.main.temp + "° !" + '</h3>' +
            '<img src="tenor.gif" width="100px" height="100px" >' +           
            '<a href="http://www.google.com/search?q=Cheap+tickets+' + json.id + '">' +
            '<h3>' + "Book a flight to " + json.id + " ?" + '</h3>' +
            '</a>' +
            '</div>'
        );
    }else{
        infoWindow.setContent(
            '<div class="infoWindow">' +
            '<h3>' + "NO, there is a "+ json.weather[0].description +" in:" + '</h3>' +            
            '<img src="rain.gif" width="100px" height="100px" >' +     
            '<h3>' + json.id + json.name + '</h3>' +  
            '</div>'
        );
    }
